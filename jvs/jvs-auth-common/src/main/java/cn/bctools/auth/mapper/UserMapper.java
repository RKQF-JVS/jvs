...
public interface UserMapper extends BaseMapper<User> {
     ...
     * @return 用户数据集合
     */
    @Select("SELECT su.*,sur.role_id FROM sys_user su LEFT JOIN sys_user_role sur ON su.id = sur.user_id WHERE sur.role_id in ${roleIds} AND su.cancel_flag = 0")
     ...
     * @return
     */
    @Select("SELECT su.* FROM  sys_user su LEFT JOIN sys_dept sd ON su.dept_id = sd.id WHERE sd.`name` = '${deptName}'  and su.cancel_flag = 0")
