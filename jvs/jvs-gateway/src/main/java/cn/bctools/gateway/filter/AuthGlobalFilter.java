...
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        //获取请求地址
        String host = getHost(exchange.getRequest());
        ...
        //根据域名获取租户，否则默认为主租户
        String tenantId = configService.fromHost(host);
    ...
    private String getHost(ServerHttpRequest request) {
        //判断是否存在代理
        if (request.getHeaders().containsKey("x-forwarded-host")) {
            return request.getHeaders().getFirst("x-forwarded-host");
        }
