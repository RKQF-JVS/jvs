        ...
        List<SwaggerResource> authRule = services.stream().filter(e -> !route.contains(e))
                .map(e -> {
                    ...
                    if (e.endsWith(SystemCons.BIZ)) {
                        swaggerResource.setLocation("/api/" + e.replaceAll(SystemCons.BIZ, "") + API_URI);
                    } else if (e.endsWith(SystemCons.MGR)) {
                        swaggerResource.setLocation("/mgr/" + e.replaceAll(SystemCons.MGR, "") + API_URI);
                    } else {
                        swaggerResource.setLocation(e + API_URI);
                    }
