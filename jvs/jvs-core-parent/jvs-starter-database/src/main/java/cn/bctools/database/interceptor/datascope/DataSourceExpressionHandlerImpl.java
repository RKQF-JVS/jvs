...
public class DataSourceExpressionHandlerImpl implements IDataSourceExpressionHandler {
    ...
    public Expression apply(Expression currentExpression, Table table) {
        ...
        if (Objects.isNull(databaseInfo)) {
            log.error(">>>> [{}] 没有找到当前数据源信息, 权限增强失败", tableName);
        ...
        if (ObjectUtils.isEmpty(tableFieldNames)) {
            log.error(">>>> [{}] 没有找到的数据表字段信息, 权限增强失败", tableName);
            return currentExpression;
        }
        UserInfoDto<UserDto> userInfo = SystemThreadLocal.get("user");
        DataScopeDto dataScope = DataScopeContextHolder.getDataScope();
        DataScopeType type = dataScope.getDataScopeType();
        if (Objects.isNull(type)) {
            log.error(">>>> [{}] 数据权限类型为空, 权限增强失败", tableName);
            return currentExpression;
        }
        if (Objects.isNull(userInfo)) {
            log.error(">>>> [{}] 当前登录用户信息为空, 权限增强失败", tableName);
            return currentExpression;
        }
        ...
        switch (type) {
            ...
            case curr_dept:
                ...
                return getExpression(currentExpression, table, DEPT_ID, hasDept, deptId);
            case curr_job:
                String jobId = user.getJobId();
                return getExpression(currentExpression, table, JOB_ID, hasJob, jobId);
            case curr_dept_tree:
                return getExpression(currentExpression, table, DEPT_ID, hasDept, childDeptIds, false);
            case self:
                String userId = user.getId();
                return getExpression(currentExpression, table, CREATE_BY_ID, hasCreateById, userId);
            case customize:
                break;
            default:
                return currentExpression;
