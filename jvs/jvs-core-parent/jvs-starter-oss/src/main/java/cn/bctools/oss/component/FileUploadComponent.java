...
public class FileUploadComponent {
    ...
    /**
     * 〈执行base64上传服务器操作 + 入库记录〉
     *   base64需要包含前缀。如data:image/png;base64,
     * @since: 1.0.0
     * @author: auto
     */
    public BaseFile doUpload(String base64DateUri, String module, String bucketName) {
        String fileType = "";
        int split = base64DateUri.indexOf(",", 0);
        String dataUri = base64DateUri.substring(0, split);
        if (dataUri.contains("png")) {
            fileType = ".png";
        } else if (dataUri.contains("svg")) {
            fileType = ".svg";
        } else if (dataUri.contains("jpg")) {
            fileType = ".jpg";
        } else if (dataUri.contains("gif")) {
            fileType = ".gif";
        }
        String fileName = UUID.randomUUID().toString() + fileType;
        String base64 = base64DateUri.substring(split + 1);
        byte[] decode = Base64.getDecoder().decode(base64);
        InputStream byteArrayInputStream = new ByteArrayInputStream(decode);

        BaseFile baseFile = getTargetTemplate().putFile(bucketName, module, fileName, byteArrayInputStream);
        return baseFile;
    }
