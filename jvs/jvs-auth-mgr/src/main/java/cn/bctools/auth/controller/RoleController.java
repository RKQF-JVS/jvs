...
public class RoleController {
    ...
    @Log
    @GetMapping("/all/tenantRole")
    @ApiOperation(value = "租户角色", notes = "角色管理, 左侧的角色列表页, 默认显示所有角色, 当前用户可查看的角色都可以看")
    public R<List<Role>> tenantRole() {
        List<Role> list = roleService.list(Wrappers.<Role>lambdaQuery().eq(Role::getType, RoleTypeEnum.tenantRole).orderByDesc(Role::getCreateTime));
        return R.ok(list);
    }
    ...
    public R<List<Role>> userRole() {
        List<Role> list = roleService.list(Wrappers.<Role>lambdaQuery().eq(Role::getType, RoleTypeEnum.userRole).orderByDesc(Role::getCreateTime));
        return R.ok(list);
    }
    ...
    public R<Boolean> tenantRole(@PathVariable("roleId") String roleId, @RequestBody List<RolePermission> list) {
        // 获取角色类型
    ...
    @Transactional(rollbackFor = Exception.class)
    public R<Boolean> dataRole(@PathVariable("roleId") String roleId, @RequestBody List<SysDataRole> dataRoles) {
        Role role = roleService.getById(roleId);
        if (role.getType().equals(RoleTypeEnum.tenantRole)) {
            //租户角色不允许赋予数据权限
            return R.failed("租户角色不能赋予数据权限");
        }
        // 清空原有数据权限
    ...
    public R<List<SysDataRole>> getDataRole(@PathVariable String roleId) {
        ...
        if (byId.getType().equals(RoleTypeEnum.userRole)) {
    ...
    /**
     * 这个值是公共的数据，初始化的公共配置数据权限值，通过注解进行扫描得到的值
     *
     * @return
     */
    @Log
    ...
    public R getDataRoles() {
        ...
        Map<String, List<DataDictDto>> descMap = dataList.stream().collect(Collectors.groupingBy(DataDictDto::getDesc));

        Map<String, List<SysData>> dataNameMap = dataService.list().stream().peek(e -> e.setDataList(descMap.get(e.getName())))
                .collect(Collectors.groupingBy(SysData::getName));
        List<Dict> dictList = new ArrayList<>();
        for (Map.Entry<String, List<SysData>> entry : dataNameMap.entrySet()) {
            dictList.add(new Dict().set("key", entry.getKey()).set("value", entry.getValue()));
        }
    ...
    public R<List<Tree<Object>>> getMenus() {
        //是否是顶级租户， 如果是顶级租户，如果是平台级管理员， 则返回所有的资源
        // 如果是普通用户， 获取用户角色信息资源。
        // 如果是下级租户，管理员， 获取上级租户已经授权的角色， 获取 资源。
        // 如果是下级租户普通用户，则获取角色信息。 对应的资源
        //获取当前登录用户的菜单信息
        //此功能，只有平台级超级管理员才能使用，其它人不能使用
//        if (!UserCurrentUtils.getCurrentUser().getPlatformAdmin()) {
//            return R.failed("平台级超级管理员才能使用此功能");
//        }
        if (!UserCurrentUtils.getCurrentUser().getAdminFlag()) {
            return R.failed("只有管理员能操作权限");
        }
        //需要获取当前租户下已有权限的菜单
        String userId = UserCurrentUtils.getUserId();
        Map<String, Apply> ids = applyService.list(Wrappers.query(new Apply().setEnable(true))).stream().collect(Collectors.toMap(Apply::getId, Function.identity()));
        ...
        Map<String, List<Menu>> menuMap = ids.values().stream()
                .flatMap(e -> {
                    Stream<Menu> stream = permissionComponent.getMenus(userId, e).stream();
                    return stream;
                })
        ...
        for (Map.Entry<String, List<Menu>> entry : menuMap.entrySet()) {
            ...
            List<TreePo> treePos = entry.getValue().stream().map(e -> BeanCopyUtil.copy(e, TreePo.class).setExtend(e))
                    .peek(e -> {
                        if (ObjectUtil.isNotEmpty(permissionMap.get(e.getId()))) {
                            e.setExtend(permissionMap.get(e.getId()));
                        }
                    })
    ...
    public R<List<TenantPo>> tenants() {
        ...
        List<TenantPo> list = tenantService.list(Wrappers.query(new TenantPo().setEnable(true).setParentId(tenantId)));
        return R.ok(list);
    }
    ...
    public R<Boolean> saveUser(@PathVariable String roleId, @RequestBody List<String> userIds) {
        ...
        //将某个用户添加到某个角色下
        String tenantId = UserCurrentUtils.getCurrentUser().getTenantId();
        Set<UserRole> collect = userIds.stream().map(e -> {
            return new UserRole().setUserId(e).setRoleId(roleId).setTenantId(tenantId);
    ...
    public R<Boolean> removeRole(@PathVariable String id) {
        ...
        if (roleService.list(Wrappers.query(new Role().setAutoGrant(true))).stream().filter(e -> e.getId().equals(id)).findFirst().isPresent()) {
            return R.failed("此角色为初始化角色不能删除");
        }
    ...
    public R<Boolean> updateRole(@RequestBody Role role) {
        //如果是初始化角色或默认角色， 不允许修改名字
        Role byId = roleService.getById(role.getId());
        //如果角色设置了为初始化角色， 那默认不能修改
        if ((SysRoleEnum.containName(byId.getRoleName()) && role.getRoleName().equals(byId.getRoleName()))) {
            return R.failed("默认角色或初始化角色不允许修改");
        }
