...
public class WxQrCodeLogin {
    ...
    @SneakyThrows
    public R<String> add(@ApiParam(value = "前端唯一id用于确认登录账号") @PathVariable("loginId") String loginId, @RequestParam(value = "client_id") String clientId) {
        AuthConfigUtil.setCurrentAppId(clientId);
        wxMpConfig.addConfigStorage(clientId);
